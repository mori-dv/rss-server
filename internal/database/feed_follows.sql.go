...
VALUES ($1, $2, $3, $4, $5, $6)
RETURNING id, created_at, updated_at, user_id, feed_id, tel_id
`
...
SELECT id, created_at, updated_at, user_id, feed_id, tel_id FROM feed_follows WHERE user_id=$1
`
...
const getFeedFollowsByTelId = `-- name: GetFeedFollowsByTelId :many
SELECT id, created_at, updated_at, user_id, feed_id, tel_id FROM feed_follows WHERE tel_id=$1
`

func (q *Queries) GetFeedFollowsByTelId(ctx context.Context, telID int32) ([]FeedFollow, error) {
	rows, err := q.db.QueryContext(ctx, getFeedFollowsByTelId, telID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
